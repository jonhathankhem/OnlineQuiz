...
<%int c=1;int a1=30;int a2=5;int x=0;String lastnum="divCheckbox1"; %>



 <script type="text/javascript">
 	var a=0;
 	var lastno="divCheckbox1";
	function fetchquestion(ac)
	{	
		return ac ;	
	  ...
	  function showhide(id) {
		  ...
		  if(lastno.localeCompare(id)==-1 || lastno.localeCompare(id)==1 )
	       	{var e = document.getElementById(id);
	       	e.style.display = (e.style.display == 'block') ? 'none' : 'block';
	       	var e = document.getElementById(lastno);
	       	e.style.display = (e.style.display == 'block') ? 'none' : 'block';
	  		}
	  	lastno=id;
	  	<%=lastnum%>=id;
...
<form action="FetchQA" method="POST">
		...
		<%int u=0;for( u=0;u<arr.size();u++) {x=u; %>
		<% Map.Entry<String, String> entry = arr.get(x); %>
		<div id="divCheckbox<%=x+1%>" style="display:<%if(u==0){out.print("block");}else{out.print("none"); }%>;">
		...
		<%String[] opt=str[1].split(";"); %>
	  	...
	  	<%out.print(entry.getValue()); %>
	  	 <button type="button" onclick="<%=entry.setValue("A")%>"  style="float:left;">Save</button>
	  	</div>
	  	<%}%>
  		</div>
		...
		<% for( int i=1; i<=Math.ceil((float)a1/a2); i++) {%>	
 			...
 			<%for(int j=1;j<=a2;j++){ if(c<=a1){%>
			    ...
			    <td> <button type="button" id="butno<%=c%>"  onclick="javascript:showhide('divCheckbox<%=c%>')"  style="width:100%;height:100%" value="<%=c %>" ><%=c%></button></td> 
			   <%c++; %>
	    	<%} }%>
 			</tr>
...
<a href=/5>5</a>
    ...
    <button type="button" onclick="javascript:showhide('<%=("divCheckbox"+Integer.valueOf(Integer.parseInt(lastnum.split("divCheckbox")[1])))%>')"  style="float:left;">Previous</button><div style="float:left;width:10px"></div><button style="float:right;" type="button" onclick="javascript:showhide('<%=("divCheckbox"+Integer.valueOf(Integer.parseInt(lastnum.split("divCheckbox")[1])+1))%>')" >Next</button>  
